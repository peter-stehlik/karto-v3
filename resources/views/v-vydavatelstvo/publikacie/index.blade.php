<x-app-layout>
        ...
        <div class="col-lg-7">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr data-id="{{ $periodical->id }}">
                        <td>{{ $periodical->id }}</td>
                        <td>{{ $periodical->name }}</td>
                        <td>{{ $periodical->abbreviation }}</td>
                        <td>{{ date('j.n.Y', strtotime($periodical->label_date)) }}</td>
                        <td>{{ number_format($periodical->price, 2, ",", " ") }} &euro;</td>
                        ...
                        <td class="text-center">
                            <a class="js-delete-periodical" data-id="{{ $periodical->id }}" data-name="{{ $periodical->name }}" data-bs-toggle="modal" data-bs-target="#deletePeriodicalModal" href="javascript:void(0);">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
    ...
    <script>
        $(document).ready(function(){
            $(document).on("click", ".js-delete-periodical", function(){
                let id = $(this).attr("data-id");
                let name = $(this).attr("data-name");

                $(".js-periodical-name").text(name);
            ...
            $(document).on("click", ".js-confirm-delete", function(){
                ...
                $.ajax({
                    url: "/vydavatelstvo/publikacie/" + id,
                    type: 'DELETE',
                    data: {
                        "id": id,
                        "_token": token,
    ...
    <!-- DELETE MODAL -->
    <!-- ------------ -->
    <div class="modal fade" id="deletePeriodicalModal" tabindex="-1" aria-labelledby="deletePeriodicalModal" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Naozaj chcete <strong>vymazať</strong> publikáciu:</p>

                    <ul>
                        <li class="js-periodical-name"></li>
                    </ul>
